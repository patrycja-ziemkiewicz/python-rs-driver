...
Callbacks come in two flavours: `on_success`/`on_error` split the two outcomes
...
def example_on_success_and_on_error(session: Session) -> None:
    """
    ...
    def handle_result(result: RequestResult) -> None:
    ...
    future = session.execute("SELECT * FROM users")
    future.on_success(handle_result)
    future.on_error(handle_error)
...
async def main() -> None:
    ...
    example_on_success_and_on_error(session)
